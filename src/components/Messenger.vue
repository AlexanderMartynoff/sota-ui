<template>
  <Layout view="hhh-ccc">
    <LayoutContent class="pt-2" ref="scrolling" :key="chat.id">
      <MessengerMessage :name="message.senderName" :message="message.text" datetime="00:00" mode="sended" v-for="message in messages">
        <template #icon>
...
import { ref, useSlots, computed, inject, nextTick, onMounted, onUnmounted, watch, useTemplateRef } from 'vue'
...
const view = computed(() => slots.sidebar ? 'shh-scc-sff' : 'hhh-ccc')
...
onMounted(() => {
  ...
  scroller.attach(scrolling.value.element, async start => {
    limit.value += 25
